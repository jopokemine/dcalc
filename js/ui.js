...
function createShareLink() {
  ...
  for (const input of allInputs) {
    if (input.value) {
      link += `&${encodeURIComponent(input.id)}=${encodeURIComponent(
        input.value
      )}`;
    }
  }
...
function recalculate() {
  ...
  const finalClassification = usingMeng
    ? toClassificationMeng(finalMark)
    : toClassification(finalMark);

  document.querySelector(
    "#finalClassification"
  ).textContent = finalClassification;
...
function gatherMarksFromPage() {
  ...
  const l6Inputs = document.querySelectorAll(
    '#l6 input:not(#fyp)[type="number"]'
  );
